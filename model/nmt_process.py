...
def get_trainingdata(data_pairs, zh_att_triples, en_att_triples):
...
def prepare_data(test_zh_true_set, test_en_true_set, zh_ent_att, en_ent_att, word2idx):


	test_encode_data, test_decode_data = get_padding_data(test_zh_true_set, test_en_true_set, en_ent_att, zh_ent_att, word2idx)
...
def get_padding_data(zh_true_set, en_true_set, en_ent_att, zh_ent_att, word2idx):
	...
	zh_ent_att_matrix, zh_total_length, zh_ent_length, encode_key_list = process_pad(zh_true_set, zh_ent_att, word2idx)

	en_ent_att_matrix, en_total_length, en_ent_length, decode_key_list = process_pad(en_true_set, en_ent_att, word2idx)
...
def process_pad(true_set, ent_att, word2idx):


	key2id = pickle.load(open('./data/att2id_fre.p', "rb"))
	...
	for i in range(len(true_set)):
		...
		for key, value_list in att_list.items():
			...
			att2id = att2id + [0] * (max_att_words - len(att2id))
			# print(att)
